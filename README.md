...
an intermediate/advanced sheet, all **CI-tested**: every Python snippet is
executed on each push, so the examples always run and never drift out of
date. Plus a `uv` tooling sheet for managing Python, projects, and
dependencies.
